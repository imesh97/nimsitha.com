...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ...
      ref={ref}
      className={cn(
        "absolute left-1.5 h-full w-px rounded-md py-3 duration-300 ease-in-out hover:bg-slate-300 rtl:right-1.5 bg-transparent",
        className
      )}
      {...props}
    />
      ...
      >
        <AccordionPrimitive.Trigger
          className={cn(
            ...
            className,
            {
              "bg-zinc-800": isSelect && isSelectable,
        ...
        <AccordionPrimitive.Content className="relative h-full overflow-hidden text-sm data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down">
          ...
          <AccordionPrimitive.Root
            dir={direction}
            type="multiple"
            className="ml-4 flex flex-col gap-1 py-1 rtl:mr-5"
            defaultValue={expandedItems}
            value={expandedItems}
            onValueChange={(value) => {
              setExpandedItems?.((prev) => [...(prev ?? []), value[0]]);
