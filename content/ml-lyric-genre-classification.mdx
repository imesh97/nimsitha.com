...
I'm used to seeing music genre classification rely heavily on the <i>audio</i> features of a song. What if we could determine a song's <i>genre</i> just by analyzing its <i>lyrics</i> instead? This is what my team and I explored in our research project, <i><b>TuneType</b></i>. 

Our research focused on distinguishing between <i>Pop</i> and <i>Rap/Hip-Hop</i> lyrics using <i>three</i> different machine learning approaches. The results were impressive! We achieved up to <i>86.69% accuracy</i> using only <i>textual</i> features (lyrics).

I'll walk you through our approach and explain our thinking behind each method, showing you how to implement <i>your own</i> lyric-based genre classifier.
...
<b>SVMs</b>, developed in the <i>1990s</i>, represent a shift from probabilistic to <i>geometric thinking</i> in AI. This approach sees classification as a geometry problem in high-dimensional space.
...
For our implementation, we used <i>Google Colab</i> and its paid <i>compute credits</i> to handle the computational demands for fine-tuning. This allowed us to train the model in a <i>reasonable timeframe</i> without investing in our own dedicated hardware.

## 📈 the evolution of approaches

These three approaches are akin to a <i>spectrum</i> in the evolution of NLP:
...
Each step <i>improves</i> how the <i>nuances</i> of language are captured, but with <i>increasing</i> computational <i>cost</i>. 

The <i>beauty</i> of our approach is that we can <i>quantify</i> and see exactly what this increased complexity buys us. <i><b>Our results:</b></i> a jump from <i>76%</i> accuracy with <b>MNB</b> to a balanced <i>81.85%</i> accuracy with <b>SVM</b>, to an impressive <i>86.69%</i> accuracy with <b>BERT</b>.
...
<b>Note:</b> We use <i>multiple</i> evaluation metrics because accuracy alone can be misleading, especially with <i>imbalanced classes</i>. The confusion matrix shows false positives and false negatives, while precision, recall, and F1-score in the classification report give a more <i>complete picture</i> of model performance for each class.

### <u>STEP 5: Analysis of Feature Importance</u> 

Understanding which features (words or phrases) are most <i>important</i> for classification provides us with some <i>valuable insights</i>.
...
- <b>MNB:</b> <i>76.00%</i> accuracy
- <b>SVM:</b> <i>81.85%</i> accuracy
- <b>BERT:</b> <i>86.69%</i> accuracy (highest performance)
...
- <b>MNB:</b> Training in seconds, <i>minimal</i> memory --> lowest accuracy
- <b>SVM:</b> Training in minutes, <i>moderate</i> memory --> balanced accuracy
- <b>BERT:</b> Training in hours (requires GPU), <i>substantial</i> memory --> highest accuracy
...
<i><b>tl;dr:</b></i> If you want to implement your own lyric-based genre classifier, then consider starting with <b>SVM</b>. It provides a <i>balance</i> between performance and efficiency, along with <i>interpretability</i> for valuable insights. For the best results, <b>BERT</b> is the way to go (but be prepared to use Google Colab).

---

If you want to learn more about our full implementation, check out the project [repository](https://github.com/imesh97/tunetype). 

You can also read our research report [here](https://github.com/imesh97/tunetype/blob/main/report.pdf).